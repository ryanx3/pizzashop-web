...
test("list orders", async ({ page }) => {
  ...
  await expect(page.getByRole("cell", { name: "Customer 1" })).toBeVisible();
...
test("paginate orders", async ({ page }) => {
  ...
  await expect(
    page.getByRole("cell", { name: "Customer 6", exact: true }),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByRole("cell", { name: "Customer 56", exact: true }),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByRole("cell", { name: "Customer 55", exact: true }),
  ).toBeVisible();
